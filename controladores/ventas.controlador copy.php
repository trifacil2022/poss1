		...
		if(isset($_POST["nuevaVenta"])){
			...
			if($respuesta == "ok"){
				...
				foreach ($listaProductos as $key => $value) {
					...
					$printer->text("L ".number_format($value["precio"],2)." Und x ".$value["cantidad"]." = L ".number_format($value["total"],2)."\n");
				...
				$printer->text("NETO: L ".number_format($_POST["nuevoPrecioNeto"],2)."\n"); //ahora va el neto

				$printer->text("IMPUESTO: L ".number_format($_POST["nuevoPrecioImpuesto"],2)."\n"); //ahora va el impuesto
